...
import com.rubber.wx.at.atp.manager.reptile.WtaRankReptile;
...
@Slf4j
@Service
public class WtaRankDataSourceService {
    ...
    public void wtaStartRankReptileTask(){
        ...
        int i=0;
        while (!wtaRankReptileSaveData(now, i * HANDLER_DATA_SIZE, HANDLER_DATA_SIZE)) {
            i++;
        }
     ...
     * @param size 当时的长度
     */
    public boolean wtaRankReptileSaveData(LocalDateTime dateTime,int index,int size){
        ...
        List<WtaRankInfoEntity> rankAddList = new ArrayList<>();
        List<WtaRankInfoEntity> rankUpdateList = new ArrayList<>();
        for (Object o:model.getData()){
            WtaRankInfoEntity rankInfoEntity = doConvertEntity((JSONObject)o);
            initNewPlayer(dateTime,rankInfoEntity);
            WtaRankInfoEntity toDayOldRanInfo = iWtaRankInfoDal.getByPlayerId(rankInfoEntity.getPlayerId(),rankInfoEntity.getDateVersion());
        ...
        if (CollUtil.isNotEmpty(rankAddList)){
            iWtaRankInfoDal.saveBatch(rankAddList);
        }
        if (CollUtil.isNotEmpty(rankUpdateList)){
            iWtaRankInfoDal.updateBatchById(rankUpdateList);
        }
        return false;
    }
     ...
     * 新增初始化
     */
    private void initNewPlayer(LocalDateTime dateTime,WtaRankInfoEntity data){
     ...
     * @return 返回数据对象
     */
    private WtaRankInfoEntity doConvertEntity(JSONObject object){
        WtaRankInfoEntity rankInfoEntity = new WtaRankInfoEntity();
