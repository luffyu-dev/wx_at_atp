...
@Slf4j
@Service
public class AtpPlayerDatasourceService {

    @Autowired
    private IAtpPlayerInfoDal atpPlayerInfoDal;
    ...
    public void startAtpPlayerInfoReptileTask()  {
        ...
        int i=0;
        while (!atpPlayerInfoReptileSaveData(i*HANDLER_DATA_SIZE,HANDLER_DATA_SIZE)){
            i++;
        }
     ...
     * @param size 当时的长度
     */
    public boolean atpPlayerInfoReptileSaveData(int index,int size){
        ...
        List<AtpPlayerInfoEntity> needAddList = new ArrayList<>();
        List<AtpPlayerInfoEntity> needUpdateList = new ArrayList<>();

        for (Object o:model.getData()){
            AtpPlayerInfoEntity  playerInfo = doConvertEntity((JSONObject)o);
            if (playerInfo == null){
                continue;
            }
            AtpPlayerInfoEntity oldPlayerInfo = atpPlayerInfoDal.getByPlayerId(playerInfo.getPlayerId());
        ...
        if (CollUtil.isNotEmpty(needAddList)){
            atpPlayerInfoDal.saveBatch(needAddList);
        }
        if (CollUtil.isNotEmpty(needUpdateList)){
            atpPlayerInfoDal.updateBatchById(needUpdateList);
        }
        return false;
    }
    ...
    private void initNewPlayer(AtpPlayerInfoEntity data){
    ...
    private boolean checkDiff(AtpPlayerInfoEntity newData,AtpPlayerInfoEntity oldData){
        boolean diff = false;
     ...
     * @return
     */
    private AtpPlayerInfoEntity doConvertEntity(JSONObject object){
        try {
            AtpPlayerInfoEntity entity = new AtpPlayerInfoEntity();
